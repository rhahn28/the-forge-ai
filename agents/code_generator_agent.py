    ...
    async def execute_step(self, state: ForgeState) -> dict:
        ...
        try:
            if command == "CREATE_DIR":
                ...
                # 🔧 CONSERVATIVE: Write content if it exists, otherwise create empty file
                ...
                if not libraries or len(libraries) == 0:
                    print("✅ No libraries to install - skipping step")
                    return {
                ...
                print(f"✅ Installed {len(libraries)} libraries")

            else:
                return {"error": f"Unknown command: {command}"}

            # 🔧 SAFE INCREMENT: Only increment if step was successful
            ...
            # 🔧 CONSERVATIVE: Handle FileSystem server errors with fallback
            # 🔧 CONSERVATIVE: Handle FileSystem server errors with fallback
            if "422 Unprocessable Entity" in str(e) or "Client error" in str(e):
                print("🚫 FileSystem MCP server error - falling back to local file creation")
                ...
                try:
                    if command == "CREATE_FILE":
                        ...
                        print(f"✅ Created file locally (fallback): {path}")
                        ...
                        with open(path, 'w', encoding='utf-8') as f:
                            f.write(content_to_write)
                        print(f"✅ Wrote file locally (fallback): {path}")
            ...
            if "returned non-zero exit status 1" in str(e) and "pip" in str(e) and "INSTALL_LIBRARIES" in command:
                print("🚫 Pip install failed - skipping library installation and continuing")
                return {
