...
Unlike chatbots such as ChatGPT, Claude, and Gemini, or coding assistants like GitHub Copilot, Windsurf, v0.dev, Bolt AI, and Cursor, The Forge is built on true Agentic AI. While the term may sound esoteric, Agentic AI simply refers to a system of specialized language models that collaborate, reason through tasks, and autonomously execute complex, multi-step workflows without requiring human prompting. By contrast, most chatbots in use today are stateless. They are good at interpreting prompts and generating one-shot responses including code, but they lack essential capabilities such as autonomous planning, iterative refinement, error detection and recovery. These limitations are not just minor technical gaps; they are fundamental blockers for using these tools for sophisticated full-stack development or complicated decision making environments. For quick research, straightfoward analysis, stateless chatbots are fine. But for sustained problem-solving, their inability to independently iterate brings progress to a halt. To help overcome these limitations, some LLM platforms such as ChatGPT have begun adopting the Model Context Protocol (MCP) devloped by Anthropic. MCP defines a standardized JSON-RPC format that allows LLMs to access external tools, databases, and software environments. This is an important step forward, and I expect it will significantly expand utility in the coming months. However, MCP alone does not confer autonomy. It simply enables structured access to tools. The underlying models still require orchestration to act agentically. If you're interested in the technical underpinnings of MCP, I recommend reviewing the official GitHub repository https://github.com/modelcontextprotocol/modelcontextprotocol.

On the other hand, true Agentic AI systems (e.g., OpenDevin, Microsoft’s AutoGen, CrewAI, LangGraph) can do autonomous planning, select tools, orchestrate tasks and iterate. I've focused on the oldest, open-source framework with the largest developer community, LangGraph. 

One additional yet critically important comment I want to make here is about a recent U.S. court ruling requiring ChatGPT to preserve and provide all user-submitted data. As a patent attorney who provides legal services to clients in a confidential manner, this is a dealbreaker. Sensitive and confidential information must remain private. Yet, a U.S. District Court judge recently ordered OpenAI to preserve all ChatGPT logs, including user-deleted conversations, in a copyright infringement lawsuit brought by The New York Times et al. The judge required that OpenAI keep all logged ChatGPT data regardless of user deletion. The privacy implications are enormous. Clearly, an on-prem, proprietary model under the full control of the firm, and hence protected under discovery is critical to maintain confidentiality. That is another major point I want to emphasize. Agentic AI must be taken out of the hands of a few large players. Firms must build on-prem, AI agentic solutions.
